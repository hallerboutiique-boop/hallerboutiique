...
test("Aurora keeps a scrollable message area and visible composer in Safari", async () => {
  ...
  assert.match(styles, /\.site-chat-profile\[hidden\],[\s\S]*?\.site-chat-conversation\[hidden\]\s*\{[\s\S]*?display:\s*none/);
  assert.match(styles, /\.site-chat-conversation\s*\{[\s\S]*?grid-template-rows:\s*auto minmax\(0, 1fr\) auto auto[\s\S]*?overflow:\s*hidden/);
