...
const translations = {
  it: {
    "meta-description": "Haller Boutique: lusso, qualita e stile.", search: "Cerca", "go-checkout": "Vai al checkout", "change-language": "Cambia lingua", "main-menu": "Menu principale", home: "Home", "new-arrivals": "Nuovi arrivi", men: "Uomo", women: "Donna", "hero-title": "LUSSO<br>QUALITÀ<br>STILE", "hero-description": "Scopri le ultime novita<br>delle migliori marche.", "discover-now": "Scopri ora", "payment-title": "Pagamento alla consegna e crypto", "payment-description": "Contrassegno o crypto<br>in sicurezza", "support-title": "Assistenza H24", "support-description": "Siamo sempre<br>disponibili", "location-banner": "Consenti l'accesso alla posizione per calcolare il tempo di consegna dalla sede di Monza.", selection: "Selezione", info: "Info", "whatsapp-support": "Assistenza WhatsApp", shipping: "Spedizioni", terms: "Termini e condizioni", "follow-social": "Seguici sui nostri social", copyright: "Copyright 2017 Haller Boutique. Tutti i diritti riservati.", sizes: "Taglie", price: "Prezzo", "add-cart": "Aggiungi al carrello", "buy-now": "Acquista ora", "try-on": "Indossa", "image-placeholder": "Immagine in arrivo"
  },
  en: {
...
const interfaceTranslations = {
  ...
  de: {
    ...
    "location-help-apple": "Wenn die Abfrage nicht erscheint, aktivieren Sie die Ortungsdienste fur den Browser in den Apple-Einstellungen und tippen Sie erneut.", "location-help-browser": "Wenn die Abfrage nicht erscheint, aktivieren Sie den Standort in den Browser-Einstellungen und tippen Sie erneut.", "location-https": "Offnen Sie die Website uber HTTPS, um den Standort freizugeben.", "location-unsupported": "Dieser Browser unterstutzt keinen Standort. Offnen Sie die Website in Safari oder Chrome.", "location-authorize": "Erlauben Sie den Standort in der Browser-Abfrage.", "location-unavailable": "Standort nicht verfugbar. Versuchen Sie es gleich erneut.", "location-active": "Standort aktiv.", "delivery-calculating": "Lieferzeit ab Monza wird berechnet...", "delivery-estimate": "Voraussichtliche Lieferung: {minutes} Minuten.", "delivery-estimate-error": "Die Lieferzeit kann nicht berechnet werden. Tippen Sie erneut.", "location-denied": "Standortzugriff wurde abgelehnt. Offnen Sie das Schloss-Symbol und erlauben Sie den Standort.", "location-gps": "Standort nicht verfugbar. Aktivieren Sie das GPS und tippen Sie erneut.", "location-error": "Standortfehler. Tippen Sie erneut.", "location-retry": "Standort nicht verfugbar. Tippen Sie erneut.",
...
translations.sq = {
  ...
  "location-help-apple": "Nese njoftimi nuk shfaqet, aktivizoni Vendndodhjen per shfletuesin te cilësimet Apple dhe prekni per te provuar perseri.", "location-help-browser": "Nese njoftimi nuk shfaqet, aktivizoni Vendndodhjen te cilësimet e shfletuesit dhe prekni per te provuar perseri.", "location-https": "Hapeni faqen me HTTPS per te lejuar vendndodhjen.", "location-unsupported": "Ky shfletues nuk mbeshtet vendndodhjen. Hapeni faqen ne Safari ose Chrome.", "location-authorize": "Lejoni vendndodhjen ne njoftimin e shfletuesit.", "location-unavailable": "Vendndodhja nuk eshte e disponueshme. Provoni perseri pas pak.", "location-active": "Vendndodhja eshte aktive.", "delivery-calculating": "Po llogaritet koha e dorezimit nga Monza...", "delivery-estimate": "Dorezimi i parashikuar: {minutes} minuta.", "delivery-estimate-error": "Koha e dorezimit nuk mund te llogaritet. Prekni per te provuar perseri.", "location-denied": "Leja e vendndodhjes u refuzua. Hapni drynin e faqes dhe vendosni Vendndodhjen ne Lejo.", "location-gps": "Vendndodhja nuk eshte e disponueshme. Aktivizoni GPS-in dhe provoni perseri.", "location-error": "Gabim vendndodhjeje. Prekni per te provuar perseri.", "location-retry": "Vendndodhja nuk eshte e disponueshme. Prekni per te provuar perseri.",
...
async function requestDeliveryEstimate(preciseLocation, requestToken) {
  setLocationBannerStatus(translate("delivery-calculating"));
  try {
    ...
    if (!response.ok || !data.ok) throw new Error(data.message || "Delivery estimate unavailable");
    if (analyticsState.locationRequestToken !== requestToken) return;
    const minutes = Number.parseInt(data.minutes, 10);
    if (!Number.isFinite(minutes) || minutes < 1) throw new Error("Invalid delivery estimate");
    setLocationBannerStatus(translate("delivery-estimate").replace("{minutes}", String(minutes)));
    sendLocationTrack("delivery_estimate", {
      deliveryMinutes: minutes,
      deliveryDistanceKm: Number(data.distanceKm || 0),
      deliveryEstimateSource: String(data.source || ""),
      deliveryOrigin: String(data.origin?.name || "Monza"),
    });
  } catch (error) {
    if (analyticsState.locationRequestToken !== requestToken) return;
    setLocationBannerStatus(translate("delivery-estimate-error"));
    sendLocationTrack("delivery_estimate_status", {
      deliveryEstimateStatus: "unavailable",
      deliveryEstimateError: error?.message || "",
    });
    analyticsState.locationRequested = false;
  }
}
...
function requestPreciseLocation(reason = "consent", options = {}) {
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      requestDeliveryEstimate(preciseLocation, requestToken);
    },
    (error) => {
      finishLocationRequest();
...
const productShareBrandIconPaths = Object.freeze({
...
function createProductShareIcon(option) {
  const path = productShareBrandIconPaths[option.id];
  return path
    ? `<svg class="product-share-brand-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="${path}"></path></svg>`
...
function createProductShareOption(option, native = false) {
  ...
  return `
    <button class="product-share-option" type="button" ${dataAttribute} data-share-brand="${escapeHtml(option.id)}">
      <span class="product-share-option-mark" aria-hidden="true">${createProductShareIcon(option)}</span>
...
function createCatalogBackMarkup() {
  return `<button class="catalog-collection-back" type="button" data-catalog-back><i data-lucide="arrow-left"></i><span>${escapeHtml(translate("catalog-back"))}</span></button>`;
}

function goBackInCatalog() {
  if (catalogState.brand) {
    catalogState = { ...catalogState, brand: "" };
  } else {
    catalogState = { gender: "", category: "", brand: "", productIds: [], view: "" };
  }
  closeCatalogNavPanels();
  renderCatalog();
  window.history.replaceState(null, "", "index.html#selezione");
...
function renderCatalog() {
  ...
  catalogRoot.innerHTML = `
    <section class="catalog-browse">
      ...
      <div class="catalog-results-heading" data-catalog-results><span>${translate("catalog-all-products")}</span></div>
...
document.addEventListener("click", (event) => {
  ...
  const catalogBack = event.target.closest("[data-catalog-back]");
...
function setupSiteChat() {
  ...
  const unlockAuroraAudio = () => {
    ...
    voiceAudio.volume = 1;
    const unlockAttempt = voiceAudio.play();
    if (!unlockAttempt?.then) {
      auroraAudioUnlocked = true;
      return;
    }
    unlockAttempt.then(() => {
      if (voiceAudio.src.includes("/assets/aurora-silence.wav")) {
        voiceAudio.pause();
  ...
  const stopVoiceRecognition = ({ discard = true } = {}) => {
    ...
    if (voiceRecognition && voiceRecognitionStarted) {
      try {
  ...
  const transcribeRecordedVoice = async (audioBlob) => {
    if (!voiceConversationActive || audioBlob.size < 200) return;
    setVoiceState("processing");
    const controller = new AbortController();
    activeTranscriptionRequest = controller;
    try {
      const formData = new FormData();
      const extension = audioBlob.type.includes("ogg") ? "ogg" : audioBlob.type.includes("mp4") ? "m4a" : "webm";
      formData.append("audio", audioBlob, `aurora-recording.${extension}`);
      ...
      if (!voiceConversationActive || error.name === "AbortError") return;
      setVoiceState("error", error.message || translate("chat-error"));
      window.setTimeout(startVoiceListening, 1200);
    } finally {
      if (activeTranscriptionRequest === controller) activeTranscriptionRequest = null;
    }
  };
  ...
  const startRecordedVoiceListening = async () => {
    ...
    try {
      ...
      voiceMediaChunks = [];
      voiceMediaRecorder = new window.MediaRecorder(
        voiceMediaStream,
        preferredType ? { mimeType: preferredType } : undefined
      );
      voiceMediaRecorder.addEventListener("dataavailable", (event) => {
        if (event.data?.size) voiceMediaChunks.push(event.data);
      });
  ...
  const startVoiceListening = () => {
    ...
    if (useRecordedVoice) {
      void startRecordedVoiceListening();
      return;
    }
    if (!voiceRecognition) {
