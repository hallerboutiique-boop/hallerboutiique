...
test("catalog navigation, stable visual search and private last-stock handling are present", async () => {
  ...
  assert.match(index, /\/assets-v\/aurora-product-previews-1\/script\.js/);
...
test("checkout keeps the home logo size inline with the header icons", async () => {
  ...
  assert.match(checkout, /\/assets-v\/aurora-product-previews-1\/styles\.css/);
  ...
  pages.forEach((html, index) => {
    const expectedStyles = pageNames[index] === "admin.html"
      ...
      : ["index.html", "account.html", "checkout.html", "product.html", "ultimi-disponibili.html"].includes(pageNames[index])
        ? /\/assets-v\/aurora-product-previews-1\/styles\.css/
...
test("Bunny receives immutable path-versioned storefront assets instead of ignored query versions", async () => {
  ...
  pages.forEach((html, pageIndex) => {
    assert.match(html, /\/assets-v\/aurora-product-previews-1\/script\.js/, scriptPages[pageIndex]);
  });
  assert.match(index, /\/assets-v\/aurora-product-previews-1\/script\.js/);
  assert.match(index, /\/assets-v\/aurora-product-previews-1\/styles\.css/);
  assert.match(checkout, /\/assets-v\/aurora-product-previews-1\/script\.js/);
  assert.match(checkout, /\/assets-v\/aurora-product-previews-1\/styles\.css/);
  ...
  assert.match(server, /"\/assets-v\/aurora-product-previews-1\/styles\.css", "\/styles\.css"/);
...
test("last-stock cards show available sizes with a visible pulse", async () => {
  ...
  assert.match(page, /\/assets-v\/aurora-product-previews-1\/script\.js/);
  assert.match(page, /\/assets-v\/aurora-product-previews-1\/styles\.css/);
