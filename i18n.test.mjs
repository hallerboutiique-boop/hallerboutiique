...
test("checkout exposes a multilingual bundle try-on", async () => {
  ...
  assert.match(checkout, /\/assets-v\/aurora-audio-3\/script\.js/);
...
test("catalog navigation, stable visual search and private last-stock handling are present", async () => {
  ...
  assert.match(script, /normalizeCatalogSearchText\(`\$\{product\.name\} \$\{product\.category\} \$\{product\.collection\} \$\{getProductBrand\(product\)\}`\)\.includes\(value\)/);
  ...
  assert.match(index, /\/assets-v\/aurora-audio-3\/script\.js/);
  ...
  assert.match(script, /function createCatalogBackMarkup\(\)/);
  assert.match(script, /function goBackInCatalog\(\)/);
  assert.match(script, /data-catalog-back/);
  assert.match(script, /if \(catalogState\.brand\)[\s\S]*?brand: ""/);
  assert.match(styles, /\.catalog-collection-back\s*\{/);
  assert.match(styles, /\.catalog-collection-back:hover/);
  ...
  assert.match(script.slice(lastStockStart, lastStockEnd), /product\.isLastAvailable && !isCatalogBagProduct\(product\)/);
...
test("checkout keeps the home logo size inline with the header icons", async () => {
  ...
  assert.match(checkout, /\/assets-v\/aurora-audio-3\/styles\.css/);
  ...
  pages.forEach((html, index) => {
    const expectedStyles = pageNames[index] === "admin.html"
      ...
      : ["index.html", "account.html", "checkout.html", "product.html", "ultimi-disponibili.html"].includes(pageNames[index])
        ? /\/assets-v\/aurora-audio-3\/styles\.css/
...
test("Bunny receives immutable path-versioned storefront assets instead of ignored query versions", async () => {
  ...
  pages.forEach((html, pageIndex) => {
    assert.match(html, /\/assets-v\/aurora-audio-3\/script\.js/, scriptPages[pageIndex]);
  });
  assert.match(index, /\/assets-v\/aurora-audio-3\/script\.js/);
  assert.match(index, /\/assets-v\/aurora-audio-3\/styles\.css/);
  assert.match(checkout, /\/assets-v\/aurora-audio-3\/script\.js/);
  assert.match(checkout, /\/assets-v\/aurora-audio-3\/styles\.css/);
  ...
  assert.match(server, /"\/assets-v\/aurora-audio-3\/script\.js", "\/script\.js"/);
  assert.match(server, /"\/assets-v\/aurora-audio-3\/styles\.css", "\/styles\.css"/);
...
test("last-stock cards show available sizes with a visible pulse", async () => {
  const [page, script, styles] = await Promise.all([
    readFile("ultimi-disponibili.html", "utf8"),
    readFile("script.js", "utf8"),
    readFile("styles.css", "utf8"),
  ]);
  assert.match(page, /\/assets-v\/aurora-audio-3\/script\.js/);
  assert.match(page, /\/assets-v\/aurora-audio-3\/styles\.css/);
...
test("product likes are public, visitor-specific and persisted by the server", async () => {
  const [script, styles, server, likesModule] = await Promise.all([
    readFile("script.js", "utf8"),
    readFile("styles.css", "utf8"),
    readFile("server.js", "utf8"),
    readFile("product-likes.mjs", "utf8"),
  ]);
  assert.equal((script.match(/\$\{createProductLikeMarkup\(product\)\}/g) || []).length, 2);
  for (const marker of [
    "data-product-like",
    "data-product-like-count",
    "loadProductLikes()",
    "toggleProductLike(likeButton)",
    '"X-Haller-Visitor": getVisitorId()',
    'fetch("/api/product-likes"',
  ]) {
    assert.match(script, new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  }
  assert.match(styles, /\.product-actions \.like-action/);
  assert.match(styles, /\.product-actions \.like-action\.is-liked svg/);
  assert.match(styles, /\.product-detail-actions \.product-community-actions\s*\{[\s\S]*?grid-column:\s*1 \/ -1/);
  assert.match(styles, /\.product-community-actions \.share-action\s*\{[\s\S]*?grid-column:\s*auto/);
  assert.match(server, /const productLikesFile = path\.join\(dataDir, "product-likes\.json"\)/);
  assert.match(server, /if \(url\.pathname === "\/api\/product-likes"\) return handleProductLikes\(req, res\)/);
  assert.match(server, /createHmac\("sha256", sessionSecret\)/);
  assert.match(server, /summarizeProductLikes\(store, visitorHash, productIds\)/);
  assert.match(server, /enqueueProductLikeMutation/);
  assert.match(likesModule, /new Set\(/);
  assert.match(likesModule, /likedProductIds/);
});

test("color variants stay grouped while preserving their own gallery, price and cart identity", async () => {
  const [index, productPage, lastStock, admin, adminJs, script, styles, server] = await Promise.all([
    readFile("index.html", "utf8"),
    readFile("product.html", "utf8"),
    readFile("ultimi-disponibili.html", "utf8"),
    readFile("admin.html", "utf8"),
  ...
  assert.match(script, /name: variantColor \? `\$\{product\.name\} – \$\{variantColor\}` : product\.name/);
  assert.match(styles, /\.product-variants\s*\{/);
  assert.match(styles, /\.product-variant-swatch\s*\{/);
  assert.match(server, /"\/assets-v\/product-variants-1\/admin\.js", "\/admin\.js"/);
});
...
test("requested Hermes products and only the unwanted men set entry are absent", async () => {
  ...
  for (const product of ["Mini Bag Hermès", "Kelly Bag Hermès", "Wallet Bag Hermès", "Dogon Wallet Hermès", "Card Holder Herm\\u00e8s"]) {
