...
test("checkout exposes a multilingual bundle try-on", async () => {
  ...
  assert.match(checkout, /\/assets-v\/admin-preview-controls-3\/script\.js/);
...
test("catalog navigation, stable visual search and private last-stock handling are present", async () => {
  ...
  assert.match(index, /\/assets-v\/admin-preview-controls-3\/script\.js/);
...
test("checkout keeps the home logo size inline with the header icons", async () => {
  ...
  assert.match(checkout, /\/assets-v\/admin-preview-controls-3\/styles\.css/);
...
test("mobile logos use collision-free layouts on every storefront page", async () => {
  ...
  pages.forEach((html, index) => {
    assert.match(html, /\/assets-v\/admin-preview-controls-3\/styles\.css/, pageNames[index]);
  });
...
test("Bunny receives immutable path-versioned storefront assets instead of ignored query versions", async () => {
  ...
  pages.forEach((html) => assert.match(html, /\/assets-v\/admin-preview-controls-3\/script\.js/));
  assert.match(index, /\/assets-v\/admin-preview-controls-3\/script\.js/);
  assert.match(checkout, /\/assets-v\/admin-preview-controls-3\/script\.js/);
  assert.match(checkout, /\/assets-v\/admin-preview-controls-3\/styles\.css/);
  assert.match(server, /const versionedPublicFiles = new Map/);
  assert.match(server, /"\/assets-v\/admin-preview-controls-3\/script\.js", "\/script\.js"/);
  assert.match(server, /"\/assets-v\/admin-preview-controls-3\/admin\.js", "\/admin\.js"/);
  assert.match(server, /"\/assets-v\/tryon-no-shoes-1\/script\.js", "\/script\.js"/);
  assert.match(server, /"\/assets-v\/admin-preview-controls-3\/styles\.css", "\/styles\.css"/);
});
...
test("admin can publish the original or cropped product image while preserving the try-on source", async () => {
  ...
  assert.match(adminHtml, /\/assets-v\/admin-preview-controls-3\/admin\.js/);
});
