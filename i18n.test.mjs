...
test("catalog navigation, stable visual search and private last-stock handling are present", async () => {
  ...
  assert.match(index, /\/assets-v\/catalog-back-1\/script\.js/);
  ...
  pages.forEach((html, index) => {
    const expectedStyles = pageNames[index] === "admin.html"
      ...
      : ["index.html", "product.html", "ultimi-disponibili.html"].includes(pageNames[index])
        ? /\/assets-v\/catalog-back-1\/styles\.css/
...
test("Bunny receives immutable path-versioned storefront assets instead of ignored query versions", async () => {
  ...
  pages.forEach((html, pageIndex) => {
    const expectedScript = scriptPages[pageIndex] === "account.html"
      ...
      : /\/assets-v\/catalog-back-1\/script\.js/;
    assert.match(html, expectedScript);
  });
  assert.match(index, /\/assets-v\/catalog-back-1\/script\.js/);
  assert.match(index, /\/assets-v\/catalog-back-1\/styles\.css/);
  assert.match(checkout, /\/assets-v\/hide-zero-stock-1\/script\.js/);
  assert.match(checkout, /\/assets-v\/admin-original-price-5\/styles\.css/);
  ...
  assert.match(server, /"\/assets-v\/catalog-back-1\/script\.js", "\/script\.js"/);
  assert.match(server, /"\/assets-v\/catalog-back-1\/styles\.css", "\/styles\.css"/);
...
test("last-stock cards show only inventory-confirmed sizes with a visible pulse", async () => {
  ...
  assert.match(page, /\/assets-v\/catalog-back-1\/script\.js/);
  assert.match(page, /\/assets-v\/catalog-back-1\/styles\.css/);
