...
test("checkout reserves independent mobile columns and gives its logo the maximum safe width", async () => {
  ...
  assert.match(checkout, /\/assets-v\/checkout-logo-large-1\/styles\.css/);
  ...
  assert.match(styles, /\.checkout-site-header \.logo\s*\{[\s\S]*?width:\s*min\(62vw, 240px\)/);
  assert.match(styles, /\.checkout-site-header \.logo\s*\{[\s\S]*?max-width:\s*calc\(100vw - 144px\)/);
...
test("Bunny receives immutable path-versioned storefront assets instead of ignored query versions", async () => {
  ...
  assert.match(server, /"\/assets-v\/checkout-logo-large-1\/styles\.css", "\/styles\.css"/);
});
